              ...
              <>
                ...
                {(user.role === 'ADMIN' || user.role === 'STAFF') && (
                  <Link
                    ...
                    className="flex items-center gap-3 px-4 py-3 hover:bg-gradient-to-r hover:from-purple-50 hover:to-pink-50 transition-colors text-purple-600 bg-gradient-to-r from-purple-50/50 to-pink-50/50"
                  >
        ...
        <div className="grid grid-cols-3 gap-0 px-4 py-2 max-w-md mx-auto">
          {navItems.map((item, index) => {
                  ...
                  <div className="absolute -top-8 left-1/2 -translate-x-1/2">
                    <div className="relative">
                      ...
                      <div className="relative w-14 h-14 rounded-full bg-gradient-to-br from-pink-500 via-purple-600 to-pink-600 shadow-2xl flex items-center justify-center text-white transform transition-all duration-300 hover:scale-110 hover:rotate-12 group-active:scale-95">
                        ...
                        <div className="relative animate-bounce-slow">{item.icon}</div>
                      </div>
                    </div>
                  </div>
            ...
            return (
              ...
              >
                <div
                  className={`transition-all duration-300 ${active ? 'scale-110 animate-bounce-once' : 'group-hover:scale-110'}`}
                >
                  {item.icon}
                </div>
                <span
                  className={`text-[10px] font-semibold mt-1 transition-all duration-300 ${active ? 'scale-105' : ''}`}
                >
                  {item.label}
                </span>
                {active && (
                  <div className="absolute -bottom-1 left-1/2 -translate-x-1/2">
                    <div className="flex gap-1">
                      ...
                      <div
                        className="w-1 h-1 rounded-full bg-pink-600 animate-pulse"
          ...
          >
            ...
            <span
              className={`text-[10px] font-semibold mt-1 transition-all duration-300 ${isMenuOpen ? 'scale-105' : ''}`}
            >
              {t('nav.account')}
            </span>
            {unreadCount > 0 && (
