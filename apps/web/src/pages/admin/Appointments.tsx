...
/** Styles for the stat tiles at the top of the page. */
const STATUS_SUMMARY_STYLES: Record<
  string,
  { bg: string; border: string; iconBg: string; textColor: string; icon: ReactNode }
> = {
  PENDING: {
...
/** Styles for the mobile appointment cards. */
...
/** Styles for the desktop table rows. */
const STATUS_ROW_STYLES: Record<
  string,
...
export default function Appointments() {
  ...
  const loadServicesAndStaff = async () => {
    try {
      ...
      setPaymentMethods(
        Array.isArray(paymentMethodsData) ? paymentMethodsData.filter((pm: any) => pm.enabled) : [],
      );
    } catch (error) {
  ...
  const handlePaymentMethodChange = async (id: string, paymentMethod: string) => {
    try {
      await api.updateAppointmentPaymentMethod(id, paymentMethod);
  ...
  const handleReschedule = async () => {
    ...
    try {
      ...
      toast.error(
        error.message || 'Failed to reschedule appointment. The time slot may not be available.',
      );
    }
  };
    ...
    <div className="space-y-6">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-4">
              ...
              <div className="space-y-2">
                ...
                <input
                  ...
                  value={filters.date}
                  onChange={(e) =>
                    setFilters({ ...filters, date: e.target.value, dateFrom: '', dateTo: '' })
                  }
              ...
              <div className="flex items-end gap-2">
                ...
                >
                  <svg
                    className={`w-4 h-4 inline-block mr-1.5 transition-transform ${showAdvancedFilters ? 'rotate-180' : ''}`}
                    fill="none"
                    stroke="currentColor"
                    viewBox="0 0 24 24"
                  >
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-3 md:gap-4">
        {STATUS_ORDER.map((status) => {
          ...
          const statusConfig = STATUS_SUMMARY_STYLES[status];
          ...
          return (
            ...
            >
              <div className="flex items-start justify-between mb-3">
                <div
                  className={`w-10 h-10 ${statusConfig.iconBg} rounded-lg flex items-center justify-center shadow-sm group-hover:scale-110 transition-transform`}
                >
                  {statusConfig.icon}
                </div>
                <span
                  className={`text-xs font-semibold px-2 py-1 ${statusConfig.textColor} bg-white/70 rounded-md`}
                >
                  {status === 'NO_SHOW'
                    ? 'No Show'
              ...
              {appointments.length > 0 ? (
                <div className="space-y-3 relative">
                  {appointments.map((apt) => {
                    const statusConfig = STATUS_CARD_STYLES[apt.status];
                    ...
                    return (
                      ...
                      >
                        ...
                        <div
                          className={`absolute left-0 top-1/2 -translate-y-1/2 w-1 h-12 ${statusConfig.iconBg} rounded-r`}
                        ></div>
                            ...
                            <div className="bg-white rounded-lg p-3 border border-gray-200">
                              ...
                              <div className="flex items-center gap-3 mt-1.5 text-xs text-gray-600">
                                ...
                                <div className="flex items-center gap-2">
                                  <input
                                    ...
                                    onChange={(e) =>
                                      handlePriceChange(apt.id, parseFloat(e.target.value))
                                    }
                              ...
                              <div className="mt-2">
                                <select
                                  ...
                                  onChange={(e) =>
                                    handlePaymentMethodChange(apt.id, e.target.value)
                                  }
            ...
            <table className="w-full">
              ...
              <tbody className="bg-white divide-y divide-gray-100">
                {sortedAppointments.map((apt) => {
                  const statusConfig = STATUS_ROW_STYLES[apt.status];
                  ...
                  return (
                    ...
                    >
                      ...
                      <td className="hidden sm:table-cell px-6 py-5">
                        <div
                          className={`inline-flex items-center gap-2 px-3.5 py-2 text-xs font-bold rounded-xl border-2 ${statusConfig.border} ${statusConfig.text} ${statusConfig.bg} shadow-sm`}
                        >
                          <span
          ...
          <div className="bg-white rounded-xl shadow-2xl max-w-md w-full p-6 animate-fade-in">
            ...
            <div className="mb-4">
              ...
              <p className="text-xs text-gray-500 mt-2">
                The system will check if the selected time is available for the staff member and
                service duration.
              </p>
            </div>
